...
function mostrarFormularioPrincipal(client, config) {
    ...
    $("#rut").rut({ formatOn: 'keyup' }); // Formateamos el campo RUT: xx.xxx.xxx-y

    $("#add-btn").on("click", function (event) { // El código que sigue se ejecuta al presionar el botón Buscar.
        event.preventDefault();
        templateHide();

        const rut = $("#rut").val(); // En rut tenemos todos los caracteres que podemos ver en el input.

        const cantidad_de_caracteres = $("#rut").val().length;
        ...
        if (cantidad_de_caracteres == 0) { // El campo RUT se encuentra vacío.
            ...
            const campo_de_busqueda = rut.split("-"); // Un RUT consta de dos secciones separadas por un -.
            ...
            if ((!isNaN(numero_correlativo) && (numero_correlativo != ""))) { // En caso cotrario se ha introducido al menos 
                ...
                if ((typeof digito_verificador != "undefined") && (digito_verificador.length != 0)) { // El usuario ha 
                    ...
                    if (RUTValido(numero_correlativo, digito_verificador)) {
                        ...
                        busquedaPorRut(numero_correlativo.concat(digito_verificador), rut, client, config);
                    }
                    else { 
...
function busquedaPorRut(rut, stringRut, client, config) {

    client.invoke('notify', 'Enviando petición de búsqueda.');
    inicioSessionZendesk(client, function (token) {

        const settings2 = {
            url: config.url + '/api/v2/users/search.json?query=rut:' + stringRut + '',
            type: 'GET',
...
function buscarUsuarioRest(rut, stringRut, config, client, token) {

    // Configuración para el acceso a la API de CBRS.
    const settings3 = {
        url: config.urlApi + rut,
        type: 'GET',
        cors: true,
        headers: {
    ...
    client.request(settings3).then(function (response) {
        ...
        crearUsuarioZendesk(response, client, config, token, stringRut, function (usuario) {
        ...
        $("#btn1").off("click").on("click", function () { // Las siguientes instrucciones serán ejecutadas si el usuario
            ...
            if (validarEmail(email) && esUnNombreValido(name) && esUnNombreValido(apellido_paterno)) {
                ...
                crearUsuarioZendesk(user, client, config, token, stringRut, function (usuario) {
                    ...
                    vaciarCampos();
                    $("#rut").val(stringRut); // Escribimos en el campo RUT del usuario actual.
...
function crearUsuarioZendesk(usuario, client, config, token, stringRUT, callback) {
    // usuario es la respuesta obtenida desde
    // CBRS o contiene la información ingresada desde nuestra interfaz de creación. 

    usuario.rutRequirente = stringRUT;
    usuario.rut = stringRUT;
...
function validarEmail(emailAddress) {
    ...
    //PatternMatch anterior: const pattern = new RegExp(/^([\w-]+@([\w-]+\.)+[\w-]{2,4})?$/);
